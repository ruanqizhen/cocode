...
在上一章中，我们完成了人机协同生产力的重要跃迁——学会了如何像一个老练的技术总监一样，指挥一支由子智能体（Subagents）构成的分布式数字团队。

然而，当你的全自动流水线开始高频运转时，你很快会撞上一个新的瓶颈：如果前端界面画歪了、UI 发生了像素级错位，或者设计师直接甩过来一张手绘的草图，你该如何向这群坐在“纯文本沙箱”里的子智能体下达指令？

在传统的结对编程中，我们和模型的交流被死死局限于“文本”。为了让 AI 帮我们还原一个精美的卡片布局，我们需要费尽心思用文字进行长篇大论的描述。这种方式不仅低效、充满歧义，而且 AI 往往词不达意，盲改出来的视觉效果更是千奇百怪。
...
│  * 剪贴板一键编码 / Handoff Bundle 接交 │  * 自动化生成录屏、截图等富媒体制品    │
│  * 黑客式的“看图写代码”极速执行者     │  * 能够自己“看”效果并自愈的测试员    │
└───────────────────────────────────────┴────────────────────────────────┘

```

### 1.1 Claude Code：以人类输入为核心的靶向视觉

虽然 Claude Code 是一款运行在纯文本终端（Terminal）中的硬核命令行工具，但它的底座 Harness 框架却拥有极其轻量且性感的图像感知力。在终端中，它主要通过两种被动方式承接人类的视觉输入：

* 剪贴板直接注入：在本地系统中利用快捷键截取 UI 缺陷或报错弹窗后，直接在终端里按下 `Cmd + V`（或 `Ctrl + V`）。Harness 会在后台全自动将图片编码并打上 `[image]` 标记推入当前的上下文窗口。
* 设计交接包（Handoff Bundle）集成：通过与 Claude Design 工具链的深度整合，设计师导出的 Figma 完整资产包可直接通过单条指令喂给终端，让主智能体瞬间获得全局像素级大局观。

### 1.2 Google Antigravity：代理自主获取的视觉反馈机制
...
这些多媒体资源会沉淀为它的制品系统（Artifacts）。你不仅可以直观地查阅它的“工作工作证明”，还可以像审阅 Google Docs 一样，直接在它提交的截图上用鼠标划线、留下批注，代理会精准提取屏幕坐标反馈并自动进行新一轮的代码修复。

---

## 2. 🎨 实战案例 1：设计稿手稿一键还原为 React 前端组件
...
## 4. 视觉提示词工程（Visual Prompt Engineering）的三大铁律
