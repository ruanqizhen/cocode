...
有些模型，只能处理文字，比如在笔者写这本书的时候，DeepSeek 还只能处理文字。但有的模型不但可以处理文字，也能处理图片，甚至音频视频，被称为多模态模型。Google Gemini 就是个多模态模型，所以我们在为自己的书编写插图的时候，甚至不需要使用额外的 AI 工具，直接在 Google Antigravity 的对话框中，提出要求，“给本书添加一些美女插图”，他就开始工作了。
