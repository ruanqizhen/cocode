...
我已经将示例中，AI 生成的知识库等上传到 Github，供读者参考： https://github.com/ruanqizhen/vanish
...
登录 Cloudflare 控制台，进入 Workers & Pages -> Create Application -> Pages -> Connect to Git，在列表中精准勾选你刚刚创建的 `vanish-book` 仓库。（新版界面文案以 Create / Connect to Git 为准，旧版“创建应用程序”已更新）。
...
AI 确实大范围地抹平了底层的编码技能鸿沟，但请一定记住：决定一套数字产品最终能走多远、拥有多高价值的，永远是屏幕前人类作者那不可被概率平均值替代的思想、经验、阅历与独特的审美。 AI 放大你的表达，而表达的灵魂来自你本身。

笔者在选择示例的时候顺手就让 AI 去写一本爽文小说，但笔者并不擅长这类题材，也没读过任何爽文小说。于是在 AI 生成一部长文之后，甚至无法判断其优劣。如果重新再选，笔者会优先考虑自己擅长的内容，可以进行长期维护。
