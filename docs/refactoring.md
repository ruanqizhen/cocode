...
```text
请为这个遗留函数编写全面的特征测试（Characterization Tests）。不要试图修复任何逻辑，你的目标是 100% 覆盖它当前的行为和所有边缘情况。这些测试将作为我重构时的安全网。
```
...
```text
【防幻觉约束】：严禁引入任何当前 `package.json` 中不存在的新第三方库。不要改变任何公开的 API 契约，不要修改与支付相关的核心逻辑。
```
