...
AI 编写代码的速度极快，但它犯错、跑偏，甚至“越帮越忙”把系统彻底搞崩的速度也同样快。如果不把每个版本的代码都保存下来，一旦 AI 在某个步骤中改坏了程序的核心逻辑，而我们又记不清它改了哪些文件、改动之前是什么样，那可就惨了——搞不好需要把整个程序推倒重来。

帮我们备份、记录、恢复每个版本代码的工具叫做版本控制工具。在所有同类工具中，Git 是当今全球软件开发中最核心、最著名的一个，而 GitHub 则是基于 Git 构建的全球最大代码托管与协作平台。
...
| **回退** | Reset / Revert / Restore | AI 时代的后悔药。Git 通过 `reset`、`revert`、`restore` 等命令实现回退（并无名为 `rollback` 的子命令），能瞬间回到上一次完美的快照状态。 |
...
Username 将会直接决定你未来在 GitHub 上的个人主页的链接前缀。例如你的用户名是 `tom`，那么你生成的网页链接就会以 `tom.github.io` 开头。也就是说，你今天随手设定的用户名，未来会成为你在网络世界里的“门牌号”。
:::
...
3. 绑定到 GitHub： 登录 GitHub → 点击右上角头像 → Settings → SSH and GPG keys → 点击 New SSH key。把刚才复制的内容粘贴进 `Key` 文本框中，起个名字（如 "My Laptop"），点击 Add SSH key。
...
当我们或 AI 修改了代码后，需要通过以下三步将代码固化为历史快照：
...
```bash
# 场景 1：AI 刚改完，你还没运行 git add，想放弃工作区的已跟踪文件修改
git restore . 
# （老版本 Git 使用：git checkout -- .）
# 注意：restore 仅影响已跟踪文件，AI 新建的未跟踪文件需用 git clean -fd 删除
# 完整清理：git restore . && git clean -fd （执行前可用 git status 和 git clean -n 预览）

# 场景 2：你已经不小心运行了 git add .，想把文件从暂存区撤回到工作区
git restore --staged .

# 场景 3：AI 连续改了好几次，把整个系统彻底改崩了，你想撤销最近的 Commit
# 【软回退】：撤销上一次 commit，但保留工作区修改，方便重新审查代码
...
:::tip
即便我们将来使用 VS Code 等自带 AI 的编程工具，而且它们本身也内置了非常漂亮的图形化 Git 面板，但在电脑里常备一个官方的 GitHub Desktop 依然是一个聪明的决定。
