...
我们常常以为，AI 作为没有实体、不需要休息的数字模型，只要给它电力和指令，它就会不知疲倦地百分之百投入工作。然而，随着你对 AI 编程工具的使用逐渐深入，你会撞上一个令人啼笑皆非的冷酷事实：AI 不仅会摸鱼，甚至还会装傻。
...
当面临大范围、重复性的代码修改时，AI 会在输出中大量使用 `// ... 保持不变 ...`。大模型其实非常聪明，它通过概率计算得出人类完全能看懂这部分被省略的旧逻辑，于是为了“省力”，直接把体力劳动原封不动地转嫁回了人类头上。
...
然而，据社区流传的一项非正式对比测试（尚未见同行评议论文）称：在面对同一批技术难题时，粗鲁版本的指令（如：“赶紧写个函数把这题搞定，别磨蹭”）其代码正确率略高于极度礼貌的版本。

> **学术严谨性说明：** 该“宾夕法尼亚州立大学研究”出处在公开论文库中难以查证，极可能是对社区轶事的误引。请以“非正式测试”的角度看待，切勿作为学术引用。核心启示是：过度的客套话会增加 Token 噪音，简洁直接往往更有效。

大模型的注意力机制根本不在乎客套话，这些词对它来说只是额外的 Token 噪音。粗鲁指令往往更短、更直接，相当于强行命令模型把全部算力集中在解决技术问题上，而不是忙着组织一段看起来很有礼貌但毫无技术含量的废话。

### 核心战术二：物质收买与恐吓（“面包板上的小费”）
...
目前，全球主流大模型的底层核心训练数据，高度依赖于 GitHub、StackOverflow 等纯英文技术社区。这导致不同语言在模型内部沉淀出了不同的“肌肉记忆”：

* 代码采纳率差异：有开发者体感报告英文技术指令在某些模型上效果更好，但官方并无公开基准，“英文比中文高 15%-30%”缺乏可靠来源，实际效果因模型和任务而异。
* Token 经济学差异：Token 消耗取决于具体分词器。现代中文优化分词器（Qwen、DeepSeek）中中文可做到约 0.6-1.0 Token/汉字，而英文为主的分词器中中文仍约 1-2 Token/汉字。所谓“中文比英文节省 40%”过于绝对，实测中英文技术描述差异通常在 ±15% 以内，建议通过 Token 计数器实测而非刻板印象。
