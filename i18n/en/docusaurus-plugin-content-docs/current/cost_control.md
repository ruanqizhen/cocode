...
- **Cache Hit Tokens (Your Profit Margin):** If the data payload sent across consecutive API requests possesses a high degree of overlap, the vendor will reuse the cached context on their servers (Prompt Caching). Each vendor has its own caching discount rules, and they change often—check the official pricing pages. Optimizing this metric is your primary financial defense!
...
In a team development environment, the ultimate waste of capital is "mindlessly operating at max power"—throwing every trivial engineering task at the most expensive, hyper-intelligent flagship models (like Claude 4 Sonnet or Gemini 2.5 Pro). 
...
| Task Tier | Recommended Model & Strategy | Relative Cost |
| --- | --- | --- |
| **Manual Labor (Completion/Comments)** | Local Models (Qwen3-Coder 14B) or Ultra-Fast Budget Models (Gemini 2.5 Flash / DeepSeek-V3) | Lowest (running locally costs next to nothing) |
| **Daily Mainstay Development** | The Cost-Effective Sweet Spot (GPT-4o-mini / Claude Haiku 4) | Low |
| **Hardcore Problem Solving** | Reasoning Models (DeepSeek-R1 / GPT-4.1) or Code Leaders (Claude Sonnet 4 / Opus 4) | Highest—draw this sword only when needed |

> **Note:** Model names and prices change fast, so this book lists no specific numbers—the table above only shows relative costs. Always check the vendors' official pricing pages.
...
The most common shortcut to bankruptcy is lazily dumping your entire project repository (including the monolithic `node_modules` folder) into the AI's prompt (commonly known as the mindless `@Codebase` command). Forcing an AI to read a 200K Token project means paying for the full 200K Tokens on every single question.

- **Tactic 1: Surgical Feeding, Reject Global RAG.** If you explicitly know where the bug is located, force the tool to read *only* the specific Git Diff and the function signatures directly involved. Explicitly tag `@src/api/user.js` instead of allowing the model to execute a blind, full-text scan of your repo.
...
- **Disable "Always-On" Auto-Completion:** Features like Cursor's Tab-Autocomplete look incredibly slick, but behind the scenes, the model is firing off a barrage of costly API requests with every keystroke. If you aren't doing deep, intensive coding, this feature can quietly burn a noticeable amount of money every day on pure noise. Reconfigure auto-completion to trigger manually via a hotkey, or delegate the autocomplete workload entirely to a free, local 14B open-source model.
...
- ❌ **The Reckless Tycoon Flow** (Defaulting to flagship models + mindless global `@Codebase` scanning + zero cache optimization): **two weeks of reckless usage can cost as much as several months of subscriptions.**
- ✅ **The Engineering Master Flow** (Tiered routing + surgical feeding + aggressive cache keep-alive): **the same work costs a fraction of that.**
...
- **Local lightweight models (Fallback/Autocomplete):** next to nothing (covers 60% of basic mechanical grunt work)
- **Mid-tier online models (Pay-As-You-Go):** low cost (handles the bulk of mainline feature development)
- **Flagship models (Emergency Refactoring):** strictly rationed (only drawn when gnawing on hardcore architectural bones)
- **Total budget:** roughly the order of a standard flat-rate subscription per month.
