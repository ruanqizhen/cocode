...
> “智能不仅在于回答问题，更在于在真实的物理与数字世界中采取行动。”
...
如果说早期的 ChatGPT 或纯聊天大模型是“军师”（问答驱动、只动嘴不动手），那么 AI Agent（人工智能智能体）就是“特工或数字化助理”。开发者只需给它一个宏观的目标，它便能自主拆解任务、规划路径、查阅资料、调用外部工具，最终交付结果。
...
| **反思机制** | 无自主反思，需用户指出错误后才会修正 | 具备自我纠错（Self-reflection）与多轮思考链（CoT） |
...
- **LangGraph：** 基于状态机（State Machine）和有向无环图（DAG）的控制流框架。它将 Agent 的每一步抽象成节点（Node）和边（Edge），非常适合构建高确定性、需要精确工程控制的后端 Agent。
...
为了完整实践上述理念，本节设计一个典型的系统级 Agent 应用——**Windows 系统命令助手**。该项目旨在通过自然语言安全地操控系统执行常见任务。
...
- ❌ *错误示范：* 封装一个 `run_terminal_command(cmd: str)` 的万能工具让 AI 直接生成 PowerShell 命令执行。这会导致**提示词攻击（Prompt Injection）**。如果恶意输入 `&& del /f /s /q C:\Windows`，系统将遭遇毁灭性打击。
