...
### 3. 专用安全分类器
在将提问送给核心的大模型之前，先经过一个轻量级的、专门经过对抗训练的安全分类器（如 Llama Guard 3 / Llama Guard 4 或 Llama Stack Safety 模块）。这类模型不具备强大的逻辑推理能力，但它们能极快且高准确率地识别出文本中是否包含“越狱”和“注入”的特征，直接从入口处拦截攻击。
