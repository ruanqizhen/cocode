...
在社交媒体时代，我们所有人的主页都长得一模一样：相同的模板、固定的格式，你的个性和内容被塞进平台统一的盒子里。但个人网站不同。在那里，页面长什么样由你决定，展示什么内容由你决定，如何向世界介绍自己也完全由你决定。
...
这一章我们将使用全球最著名的代码托管平台——GitHub 提供的免费静态网站服务——GitHub Pages。你无需租用服务器，无需购买域名，也无需学习复杂的运维知识。只需要点几下鼠标，5 分钟后，你就会拥有一个真正运行在互联网中的个人主页。
...
AI 会替我们把这些抽象的审美翻译成真正运行的网页代码。下面是一个极简素雅的个人主页提示词模板，你可以直接复制给 AI：
...
（当我们需要向 AI 传递更复杂的要求时，通常会使用 Markdown 格式来组织和书写文字。关于这种格式的介绍，请参考：[附录：Markdown 文档](markdown)）
...
1. 零首屏闪烁：在 `<head>` 顶部嵌入遮罩逻辑或立即执行函数 (IIFE)，确保页面首次加载或刷新时，主题（暗黑/亮色）瞬间匹配用户系统或 `localStorage`，绝不允许出现一瞬间的白屏或闪烁。
...
无论你使用的是 ChatGPT Canvas、Claude Artifacts、Gemini、Meta AI，还是其他 AI 工具，只要它根据上面的提示词生成了网页程序，你最终都需要把这段代码变成你电脑里的一个真实文件。
...
有了这个本地的 `index.html` 个人主页文件后，将它发布到互联网其实远没有想象中那么复杂，整体流程大概只有下面四步：
...
但如果你将仓库名精准设置为 `你的用户名.github.io`，GitHub 会认定这是你的**最高级别个人主页**，最终生成的网址将是极其完美的：**`https://tom.github.io/`**。后面没有任何冗余路径，瞬间让你拥有大厂官网般的专业高级感！
:::
...
3. **点击链接**：点击那个网址，奇迹发生了！你亲手指挥 AI 编写的、极具设计感的个人主页，已经在真实的互联网上完美运行了！
...
* **网络资源引用**：如果你在网页中让 AI 引入外部的非自定义资源，确保使用的是**网络绝对链接**（例如 `https://picsum.photos/200`）。若要使用自己独有的本地图片，请务必参考第四步的“高阶玩法”使用相对路径并一同上传。
